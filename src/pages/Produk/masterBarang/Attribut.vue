    ...
    <div class="content-wrapper">
        <div id="master-barang">
            <div class="page-header">
                <div class="page-header-content">
                    <div class="page-title clearfix">
                        <h4 class="float-left">Atribut</h4>
                        <div class="add-actions float-right" v-if="$dataRole['attribute.create']">
            ...
            <div class="content">
                    ...
                    <div>
                        ...
                        <div class="" v-if="itemsData">
                            ...
                            <el-table 
                                ...
                                v-loading="loading"
                                class="mobayar-table">
                                <el-table-column type="selection" width="55" v-if="$dataRole['attribute.delete']"></el-table-column>
                            ...
                            <div class="base-template-sidebar">
                                ...
                                <div class="actions-button clearfix">
                                    <el-button v-if="$dataRole['attribute.delete']" v-show="(formsType == 'edit')" class="button-outline-black" id="trash" @click.prevent="deletePost('once', itemDataDetail.id)">
                                    ...
                                    <el-button v-if="$dataRole['attribute.edit']" v-show="(formsType == 'edit')" class="button-white-green" id="save-edit" @click.prevent="submitSave('formsvalid', 'edit')">Simpan</el-button>
                ...
                <!-- <attribute-create ref="createAttr" :id="dataId"></attribute-create> -->
            </div>
        </div>
    </div>
</template>
...
<script>
    ...
    // import AttributeCreate from './AttributeCRUD.vue'
    ...
    export default {
        ...
        data () {
            var validateCode = (rule, value, callback) => {
                if (value === '') {
                    ...
                    if (/^[a-zA-Z0-9- ,_]*$/.test( this.forms.key ) == false) {
                        callback(new Error('Key tidak boleh ada symbol'));
                    }
                    callback();
                }
            };
            var validateCode2 = (rule, value, callback) => {
                if (value === '') {
                    ...
                    // if (/[^a-zA-Z0-9\-\/]/.test( this.forms.value )) {
                    //     callback(new Error('Value tidak boleh ada symbol'));
                    // }
                    callback();
                }
            };
            return {
                ...
                rules2:{
                    key: [ { required: true, validator: validateCode, trigger: 'blur' } ],
                    value: [ { required: true, validator: validateCode2, trigger: 'blur' } ],
                },
        ...
        created(){
            FUNCTIONS.checkPermissions(this.$dataRole['attribute.list'])
        },
        mounted () {
        ...
        methods: {
            ...
            deletePost(type, valueid){
                ...
                if(type == 'all'){
                    ...
                    if(this.itemiDforDelete.length <= 5){
                        this.$confirm('Apakah anda yakin akan menghapus '+ id.length +' item dari daftar?', 'Hapus Atribut', {
                    ...
                    this.$confirm('Apakah anda yakin akan menghapus item dari daftar?', 'Hapus Atribut', {
                        confirmButtonText: 'OK',
                        cancelButtonText: 'Cancel',
                        type: 'warning'
                    }).then(() => {
                        ...
                        if(formtype == 'save'){
                            ...
                            .then(response => {
                                ...
                                if(response.data.status == 'error'){
                                    ...
                                    this.messagesuccess = response.data.data.value + " Berhasil ditambahkan."
                                    this.getPosts(this.perPage, this.currentPage, '')
                                    this.closeModal();
                                }
                            })
                            ...
                            .then(response => {
                                ...
                                if(response.data.status == 'error'){
                                    ...
                                    this.messagesuccess = response.data.data.value + " Berhasil diubah."
                                    this.getPosts(this.perPage, this.currentPage, '')
                                    this.closeModal();
                                }
                            })
            ...
            handleCurrentChange(val){
                if(this.$dataRole['attribute.show']){
                    if(val){
                        document.querySelector('.base-template-sidebar').classList.add('showing');
                        document.querySelector('.backdrop').classList.add('show');
                        document.querySelector('body').classList.add('no-scroll');
                        this.formsType = 'edit'
                        this.titleForms = "Edit Atribut"
                        this.itemDataDetail = val
                        this.forms.key = val.key
                        this.forms.value = val.value

                        // this.dataId = this.itemDataDetail.id
                    }
                }else{
                    this.$notify.error({
                        title: 'Error',
