    ...
    <div class="content-wrapper">
        <div id="master-barang-category-product">
            <div class="page-header">
                <page-header titleName="Produk Offline Hub" :isCreate="$dataRole['service.offlinehub.post.product.create']"></page-header>
            </div>
            <div class="content">
                ...
                <el-table 
                    ...
                    <el-table-column prop="name" label="Nama" width="190"></el-table-column>
                    ...
                    <el-table-column label="Deskripsi">
                        <template slot-scope="scope">
                            <less-text :text="scope.row.description" :max-chars="150"></less-text>
                        </template>
                    </el-table-column>
                    <el-table-column label="Last Update" width="250">
                        <template slot-scope="scope">
                ...
                <div class="base-template-sidebar">
                                ...
                                <div class="item-forms-details">
                                    ...
                                    <div class="forms-details">
                                        <el-row :gutter="20">
                                            ...
                                            <el-col :span="12">
                                                <el-form-item label="Kategori Produk" prop="categoryId">
                                                    <el-select no-data-text="No data" v-model="forms.categoryId" placeholder="Select">
                                                        <el-option v-for="(dataCat, indexCat) in itemsDataCategoryProduct" :value="dataCat.id" :label="dataCat.name" :key="indexCat">{{dataCat.name}}</el-option>
                                                    </el-select>
                                                </el-form-item>
                                            </el-col>
                                            <!-- <el-col :span="24">
                    ...
                    <div class="actions-button clearfix">
                        <el-button v-if="$dataRole['service.offlinehub.delete.product.delete.*']" v-show="(formsType == 'edit')" class="button-outline-black" id="trash" @click.prevent="deletePost(itemDataDetail.id)">
                        ...
                        <el-button v-if="$dataRole['service.offlinehub.put.product.update.*']" v-show="(formsType == 'edit')" class="button-white-green" id="save-edit" @click.prevent="submitSave('formsvalid', 'edit')">Simpan</el-button>
...
export default {
    name: 'offline-hub-product',
    data(){
        return{
            sectionApi: 'offlinehub?servicePathUrl=product',
            sectionApiCategoryProduct: 'offlinehub?servicePathUrl=category',
            loading: false,
            loadingCategoryProduct: false,
            loadingForm: false,
            itemsData: [],
            itemsDataCategoryProduct: [],
            itemDataDetail: [],
            searchQuery: '',
            totalPage: '',
            perPage: 10,
            ...
            messagesuccess: '',
            messageerror: '',
            dataTextCreate: 'Buat Produk Offline Hub',
            dataTextUpdate: 'Edit Produk Offline Hub',
            confirmDescDelete: 'Anda yakin akan menghapus data Produk dari daftar?',
            confirmTitleDelete: 'Hapus Produk',
            textDataCreate: 'Data Produk Berhasil ditambahkan.',
            textDataUpdate: 'Data Produk Berhasil diupdate.',
            ...
            rules:{
                ...
                categoryId: [ { required: true, message: 'Inputan harus di isi', trigger: 'change' } ],
            }
        }
    },
    watch:{
    ...
    created(){
        FUNCTIONS.checkPermissions(this.$dataRole['service.offlinehub.get.product'])
    },
    mounted(){
        this.getPost(this.searchQuery)
        this.getPostCategoryProduk()
    ...
    methods:{
        ...
        getPostCategoryProduk(){
            this.loadingCategoryProduct = true
            let url = this.urlService + this.sectionApiCategoryProduct + '&limit=100' + '&page=' + this.currentPage + '&direction=DESC'
            this.axios.get(url)
            .then(response => {
                if(response.data.status == 'success'){
                    this.itemsDataCategoryProduct = response.data.data.rows
                }
            })
            .catch(error => {
                this.$notify.error({
        ...
        handleCurrentChange(val){
            if(this.$dataRole['service.offlinehub.get.product.detail.*']){
                if(val){
            ...
            if(type == 'post'){
                ...
                .then(response => {
                    if(response.data.status == 'success'){
                        ...
                        this.forms.categoryId = this.itemDataDetail.category.id
