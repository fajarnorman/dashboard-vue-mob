            ...
            <div class="page-header">
                <div class="page-header-content">
                    <div class="page-title clearfix">
                        <h4 class="float-left">Daftar Pelanggan</h4>
                        <div class="add-actions">
                            ...
                            <a @click.prevent="Create" class="float-right" style="margin-right: 15px;" v-if="$dataRole['customer.create']">
                    ...
                    <div>
                        ...
                        <div class="" v-if="itemsData">
                            ...
                            <el-table 
                                ...
                                v-loading="loading"
                                class="mobayar-table">
                                <el-table-column type="selection" width="55" v-if="$dataRole['customer.delete']"></el-table-column>
                            ...
                            <div class="base-template-sidebar">
                                                    ...
                                                    <el-row :gutter="20">
                                                        ...
                                                        <el-col :span="12">
                                                            <el-form-item label="Nomor Telepon" prop="no_telp">
                                                                <el-input type="number" v-model="forms.no_telp" class="custom-validnum"></el-input>
                                                            </el-form-item>
                                                        </el-col>
                                                        <el-col :span="12">
                                            ...
                                            <div class="item-forms-details" v-show="(formsType == 'edit')">
                                                ...
                                                <div class="forms-details">
                                                    <el-row :gutter="20">
                                                        ...
                                                        <el-col :span="12" v-if="itemDataDetail.status_upgrade !== ''">
                                ...
                                <div class="actions-button clearfix">
                                    <el-button v-show="(formsType == 'edit')" v-if="$dataRole['customer.delete']" class="button-outline-black" id="trash" @click.prevent="deletePost('once', itemDataDetail.id)">
                                    ...
                                    <el-button v-show="(formsType == 'edit')" v-if="$dataRole['customer.edit']" class="button-white-green" id="save-edit" @click.prevent="submitSave('formsvalid', 'edit')">Simpan</el-button>
...
<script>
    ...
    export default {
        ...
        created(){
            FUNCTIONS.checkPermissions(this.$dataRole['customer.list'])
        },
        mounted(){
            // Get all Post user
            // this.getPosts(this.perPage, this.currentPage, this.searchQuery, this.filterQuery, this.dateRange[0], this.dateRange[1])
            ...
            JQuery(".custom-validnum .el-input__inner").bind({
                keydown: function(e) {
                    if (e.shiftKey === true ) {
                        if (e.which == 9) {
                            return true;
        ...
        watch: {
            ...
            filterQuery: function(newFilter){
                this.getPosts(this.perPage, this.currentPage, this.searchQuery, newFilter, this.dateRange[0], this.dateRange[1])
                document.querySelector('.el-pagination').children[1].children[0].click()
            },
            dateRange: function(newdateRange){
                this.getPosts(this.perPage, this.currentPage, this.searchQuery, this.filterQuery, newdateRange[0], newdateRange[1])
                document.querySelector('.el-pagination').children[1].children[0].click()
            },
        ...
        methods: {
            // build url function
            buildUrl (limit, page, search, status, dateStart, dateEnd) {
                return this.apiUrl + this.sectionApi + "?limit=" + limit + "&page=" + page + '&search=' + search + '&role=' + this.role + '&status=' + status + '&date_start=' + dateStart + '&date_end=' + dateEnd
            },
            ...
            // get all post
            getPosts(num, pg, sch, sts, dtstart, dtend){
                let url = this.buildUrl(num, pg, sch, sts, dtstart, dtend)
                this.loading = true
                this.axios.get(url)
                .then(response => {
            ...
            getClaimItems(){
                ...
                .catch(error => { 
                    this.countErrorGetClaimItems++
                    if(this.countErrorGetClaimItems >= 3){
                        this.errored = true
                    }else{
                        this.getClaimItems()
                    }
            ...
            getRolePermission(){
                ...
                .catch(error => { 
                    this.countErrorGetRolePermission++
                    if(this.countErrorGetRolePermission >= 3){
                        this.errored = true
                    }else{
                        this.getRolePermission()
                    }
            ...
            deletePost(type, valueid){
                if(type == 'all'){
                    ...
                    this.$confirm('Apakah anda yakin akan menghapus '+ id.length +' items dari daftar?', 'Hapus Pelanggan', {
                    ...
                    this.$confirm('Apakah anda yakin akan menghapus '+ this.itemDataDetail.fullname +' dari daftar?', 'Hapus Pelanggan', {
                        confirmButtonText: 'OK',
                        cancelButtonText: 'Cancel',
                        type: 'warning'
                    }).then(() => {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        if(formtype == 'save'){
                            ...
                            .catch(error => { 
                                console.log(error)
                                // FUNCTIONS.notify('error', 'Error', error)
                            })
                        }else{
            ...
            getExcel(){
                ...
                if((getYear_1 - getYear_2) == 0){
                    if((getDateMonth_1 - getDateMonth_2) == 0){
                        ...
                        let url = this.buildUrl(this.totalPage, this.currentPage, this.searchQuery, this.filterQuery, this.dateRange[0], this.dateRange[1])
                        ...
                        .finally(() => {
                                ...
                                exportToExcel.push(
                                    {
                                        ...
                                        'Status Upgrade': currentValue.status_upgrade,
                                        'Created at' : currentValue.created_at,
                                    }
                                )
                            })

                            let filename = "Pelanggan"
                            ...
                            XLSX.utils.book_append_sheet(wb, datas, 'Data Pelanggan')
